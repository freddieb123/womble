  ...
  const startRecording = async () => {
    try {
      ...
      if (!MediaRecorder.isTypeSupported(options.mimeType)) {
        console.log(
          "Preferred mime type not supported, falling back to browser default",
        );
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          errorMessage =
            "Your microphone is busy or not working properly. Please close other apps using your microphone.";
          errorTitle = "Microphone Busy";
        } else if (error.name === "SecurityError") {
          errorMessage =
            "Your browser's security settings blocked microphone access. Try using HTTPS or a different browser.";
    ...
    try {
      ...
      if (!saveResponse.ok) {
        ...
        if (saveResponse.status === 413) {
          throw new Error(
            "Audio file is too large to upload. Try a shorter recording.",
          );
        } else if (saveResponse.status === 415) {
          throw new Error(
            "Unsupported audio format. Please try again with a different browser.",
          );
        } else if (saveResponse.status === 400) {
          throw new Error(
            errorData.message ||
              "Invalid request: " + (errorData.error || "unknown error"),
          );
        } else {
          throw new Error(
            errorData.message ||
              "Failed to save audio: " + (errorData.error || "Server error"),
          );
        }
      }
      ...
      const transcribeResponse = await fetch(
        `/api/dual-conversation/transcribe?${transcribeParams.toString()}`,
        {
          method: "POST",
          headers: {
      ...
      toast({
        title: "Success!",
        description:
          "Your conversation has been processed and is ready for feedback.",
      });
    } catch (error) {
      ...
      if (errorMessage.includes("quota")) {
        ...
        errorMessage =
          "The OpenAI API quota has been exceeded. Please add a payment method to your OpenAI account or wait until the quota resets.";
      } else if (errorMessage.includes("too large")) {
        errorTitle = "File Too Large";
        errorMessage =
          "The recording exceeds the maximum file size. Try recording a shorter conversation.";
      } else if (
