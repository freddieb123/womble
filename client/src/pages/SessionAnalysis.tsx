...
export default function SessionAnalysis() {
  ...
  const deepLinkConfigId = searchParams.get('configId') ? parseInt(searchParams.get('configId')!) : null;
  const [selectedConfigId, setSelectedConfigId] = useState<number | null>(deepLinkConfigId);
...
function QuickFireQuizAnalysis({ configId }: { configId: number }) {
  ...
  const { data: participantResponses } = useQuery<{ questions: any[] }>({
    queryKey: ["/api/quick-fire-quiz", configId, "participant-responses"],
    queryFn: async () => {
      const res = await fetch(`/api/quick-fire-quiz/${configId}/participant-responses`);
      if (!res.ok) return { questions: [] };
      return res.json();
    },
    enabled: showReview,
    refetchInterval: showReview ? 15_000 : false,
  });
    ...
    <div className="space-y-4">
      ...
      {showReview && questions.length > 0 && (
        <div className="mt-2 space-y-3">
          ...
          {(participantResponses?.questions ?? questions.map((q: any) => ({ ...q, questionId: q.id, question: q.question, participants: [] }))).map((q: any, qi: number) => {
            const isExpanded = expandedQuestions.has(q.questionId ?? q.id);
            const participantCount = q.participants?.length ?? 0;
            const correctCount = q.participants?.filter((p: any) => p.correct).length ?? 0;
            return (
              <div key={q.questionId ?? q.id} className="border border-gray-100 rounded-lg bg-white overflow-hidden">
                {/* Question header + options */}
                <div className="p-3 space-y-2">
                  <p className="text-sm font-semibold text-gray-800">Q{qi + 1}. {q.question}</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    {(q.options ?? []).map((opt: string, oi: number) => {
                      const isCorrect = oi === (q.correctIndex ?? q.correct_index);
                      return (
                        <div
                          key={oi}
                ...
                {/* Participant answers toggle */}
                {participantCount > 0 && (
                  <button
                    onClick={() => toggleQuestion(q.questionId ?? q.id)}
                    className="w-full flex items-center justify-between px-3 py-2 bg-gray-50 border-t border-gray-100 text-xs text-gray-500 hover:bg-gray-100 transition-colors"
                  >
                    <span>{correctCount}/{participantCount} got it right</span>
                ...
                {/* Per-participant answer rows */}
                {isExpanded && (
                  <div className="divide-y divide-gray-50">
                    {q.participants.map((p: any, pi: number) => {
                      const chosenOption = (q.options ?? [])[p.selectedIndex];
                      return (
                        <div key={pi} className={`flex items-center gap-3 px-3 py-2 text-sm ${p.correct ? 'bg-green-50' : 'bg-red-50'}`}>
                          <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${p.correct ? 'bg-green-500 text-white' : 'bg-red-400 text-white'}`}>
                            {p.correct ? '✓' : '✗'}
                          </span>
                          <span className={`flex-1 font-medium ${p.correct ? 'text-green-800' : 'text-red-800'}`}>{p.userName}</span>
                          <span className={`text-xs ${p.correct ? 'text-green-600' : 'text-red-500'}`}>{chosenOption ?? `Option ${LABELS[p.selectedIndex]}`}</span>
                        </div>
                      );
                    })}
                  </div>
